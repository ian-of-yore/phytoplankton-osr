# (keep EXACT content you wrote — unchanged)
import os
import random
...
def cluster_unknowns_from_openmax_logits_gate(
    ...
    test_p = openmax_probs_logits_saved(test_logits, mavs, weibulls, alpha=alpha_eff, dist=dist_name)
    punk_all = test_p[:, -1]

    sel_mask, tau_eff, k = select_topq_tiesafe(punk_all, q_unknown=q_unknown)
